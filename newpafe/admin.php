...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_profile') {
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM user_profile WHERE email = ? AND user_id != ?");
    ...
    $stmt = $pdo->prepare("
        INSERT INTO user_profile (user_id, full_name, email, profile_picture, gender, year_level, section_name)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_report') {
    ...
    try {
        if ($report_type === 'events') {
            ...
            $stmt->execute([$user_profile['full_name']]);
            $data = $stmt->fetchAll(PDO::FETCH_ASSOC);
            foreach ($data as $row) {
                fputcsv($output, [
                    $row['event_name'] ?? '',
...
if ($selected_event_id) {
    try {
        ...
        ");
        $stmt->execute([$selected_event_id, $user_profile['full_name']]);
